# From <https://github.com/miinay/SSD_distribution_comparison/blob/main/Rcode.R>
...
EnviroTox_ssd$ConsensusMoA <- replace(
  EnviroTox_ssd$ConsensusMoA,
  which(EnviroTox_ssd$ConsensusMoA == "N"),
  "Narcotic"
)
EnviroTox_ssd$ConsensusMoA <- replace(
  EnviroTox_ssd$ConsensusMoA,
  which(EnviroTox_ssd$ConsensusMoA == "U"),
  "Unclassified"
)
EnviroTox_ssd$ConsensusMoA <- replace(
  EnviroTox_ssd$ConsensusMoA,
  which(EnviroTox_ssd$ConsensusMoA == "S"),
  "Specifically acting"
)
...
temp.res <- data.frame(matrix(
  -9999,
  ncol = 1 + 8 * 4 + 6 * 4,
  nrow = length(StudyChemicals_A)
))
head(temp.res)

for (i in 1:length(StudyChemicals_A)) {
  Temp.data <- EnviroTox_test_selected2 %>%
    filter(Test.type == "A" & original.CAS == StudyChemicals_A[i])
  # fit distributions
...
res_aic_wider <- res_aic %>%
  ...
  group_by(original.CAS) %>%
  pivot_wider(
    id_cols = original.CAS,
    names_from = dist,
    values_from = c("ad", "ks", "cvm", "aic", "aicc", "bic", "delta")
  )
...
res_hc_wider <- res_hc %>%
  ...
  pivot_wider(
    id_cols = original.CAS,
    names_from = dist,
    values_from = c("percent", "est", "se", "lcl", "ucl"),
    values_fill = NA
  )

temp.res_acute_clean <- left_join(
  res_aic_wider,
  res_hc_wider,
  by = "original.CAS"
)
...
temp.res_acute_clean_2 <- left_join(
  temp.res_acute_clean,
  d02_A,
  by = "original.CAS"
)
...
for (i in 1:length(StudyChemicals_C)) {
  Temp.data <- EnviroTox_test_selected2 %>%
    filter(Test.type == "C" & original.CAS == StudyChemicals_C[i])
  fits <- ssdtools::ssd_fit_dists(
    Temp.data,
    left = 'Effect.value',
    dists = c('lnorm', 'llogis', 'burrIII3', 'weibull'),
...
temp.res_chronic_clean <- left_join(
  res_aic_wider,
  res_hc_wider,
  by = "original.CAS"
)
...
# Add the information of each compound such as MoAs
temp.res_chronic_clean_2 <- left_join(
  temp.res_chronic_clean,
  d02_C,
  by = "original.CAS"
)
...
res_acute <- temp.res_acute_clean_3 %>%
  ...
  mutate(
    AICcdif_llogis = aicc_llogis - aicc_lnorm,
    AICcdif_burrIII3 = aicc_burrIII3 - aicc_lnorm,
    AICcdif_weibull = aicc_weibull - aicc_lnorm
  ) %>%
  mutate(
    HC5rat_llogis = ifelse(
      est_llogis == "NULL",
      NA,
      ifelse(
        est_lnorm == "NULL",
        NA,
        as.numeric(unlist(est_llogis)) / as.numeric(unlist(est_lnorm))
      )
    ),
    HC5rat_burrIII3 = ifelse(
      est_burrIII3 == "NULL",
      NA,
      ifelse(
        est_lnorm == "NULL",
        NA,
        as.numeric(unlist(est_burrIII3)) / as.numeric(unlist(est_lnorm))
      )
    ),
    HC5rat_weibull = ifelse(
      est_weibull == "NULL",
      NA,
      ifelse(
        est_lnorm == "NULL",
        NA,
        as.numeric(unlist(est_weibull)) / as.numeric(unlist(est_lnorm))
      )
    )
  )
...
res_acute_selected1 <- res_acute %>%
  ...
  pivot_longer(
    cols = c(AICcdif_llogis, AICcdif_burrIII3, AICcdif_weibull),
    names_to = "dist",
    names_prefix = "AICcdif_",
    values_to = "AICcdiff"
  )
...
res_acute_selected2 <- res_acute %>%
  ...
  pivot_longer(
    cols = c(HC5rat_llogis, HC5rat_burrIII3, HC5rat_weibull),
    names_to = "dist",
    names_prefix = "HC5rat_",
    values_to = "HC5_ratio"
  )

res_acute_visualization <- left_join(
  res_acute_selected1,
  res_acute_selected2,
  by = c("original.CAS" = "original.CAS", "dist" = "dist")
)

res_acute_visualization2 <- left_join(
  res_acute_visualization,
  d02_A,
  by = "original.CAS"
)
...
res_chronic_visualization <- left_join(
  res_chronic_selected1,
  res_chronic_selected2,
  by = c("original.CAS" = "original.CAS", "dist" = "dist")
)

res_chronic_visualization2 <- left_join(
  res_chronic_visualization,
  d02_C,
  by = "original.CAS"
)
...
Fig_AICdiff_Acute <- res_acute_visualization3 %>%
  mutate(across(
    dist2,
    factor,
    levels = c("Log-logistic", "Burr type III", "Weibull")
  )) %>%
  ggplot(aes(dist2, AICcdiff, alpha = 0.7)) +
  geom_boxplot() +
  geom_line(
    aes(group = original.CAS),
    size = 0.5,
    color = 'gray',
    alpha = 0.6
  ) +
  geom_point(
    aes(color = ConsensusMoA, shape = ConsensusMoA, group = original.CAS),
    size = 2.5,
    alpha = 0.7
  ) +
  scale_fill_manual(
    values = c(
      "Narcotic" = "olivedrab1",
      "Specifically acting" = "lightskyblue",
