...
EnviroTox_ssd <- aggregate(
  ...
  mutate(sd = aggregate(EnviroTox_test_selected2$Effect.value,
    by = list(EnviroTox_test_selected2$original.CAS, EnviroTox_test_selected2$Test.type), function(x) sd(log10(x))
  )[, 3]) %>%
  dplyr::rename(mean = x) %>%
  ...
  mutate(No_species = aggregate(EnviroTox_test_selected2$Latin.name,
    by = list(EnviroTox_test_selected2$original.CAS, EnviroTox_test_selected2$Test.type), function(x) length(unique(x))
  )[, 3]) %>%
...
envirotox_acute <- EnviroTox_test_selected2 %>%
  ...
  mutate(Iwasaki25 = Iwasaki25 & !(Short_name %in%
    c(
      "Arsenic(III) sulfide", "Cadmium chloride", "Chromium trioxide", "Copper",
